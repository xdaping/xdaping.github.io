...
class ClassifyModel(models.Model):
    ...
    def __init__(self, feature_columns):
        ...
        self.embed_layers = {'embed_' + str(i): Embedding(feat['feat_num'], 4) for i, feat in enumerate(self.sparse_feature_columns)}
...
def run_model(model, features, label, training=False):
    ...
    loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(label, pred)) # 该方法效果更好更稳定
    #loss = tf.reduce_mean(tf.losses.mean_squared_error(label, pred))
    # 将预测概率转换为二进制值（0或1），基于0.5的阈值
    pred_binary = tf.cast(tf.greater(pred, 0.5), tf.float32)
    acc = tf.reduce_mean(tf.cast(tf.equal(label, pred_binary), tf.float32))
