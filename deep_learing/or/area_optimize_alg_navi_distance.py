...
"""
@software: PyCharm
@file: area_optimize_alg_navi_distance.py
...
class OptimizeAlg(object):
    ...
    def _init_var(self):
        ...
        for i in self.i_range:
            # 基础集合固定都选，不需要决策变量
            if self.data[i]['navi_cls'] == 1:
                self.v[i, 0] = 0
                self.v[i, 1] = 1
            # 不在基础集合和候选集合固定都不选，不需要决策变量
            elif self.data[i]['navi_cls'] == 4:
                self.v[i, 0] = 0
                self.v[i, 1] = 0
            else:
                for j in self.j_range:
    ...
    def _set_obj(self):
        ...
        self.solver.Minimize(
            sum([sum([self.data[i]['prediction_map']['dist_navi'][j] * self.v[i, j] for j in self.j_range]) for i
                 in self.i_range]))
    ...
    def _add_constraint(self):
        ...
        for i in self.i_range:
            ...
            # 在候选集合里面的只有一个treatment（选或者不选），剩下的流向不需要约束
            if self.data[i]['navi_cls'] == 2 or self.data[i]['navi_cls'] == 3:
                self.solver.Add(sum([self.v[i, j] for j in self.j_range]) == 1)
            # 只属于基础集合的商家必选
            # if self.data[i]['base_set'] == 1 and self.data[i]['candidate_set'] == 0:
            #     for j in self.j_range:
        ...
        # 单量约束，基于导航距离的新集合单量和缩圈的单量进行对齐
        order_threshold = -0.003
        ...
        adjust_order_cnt = sum(
            [sum([self.data[i]['prediction_map']['order_navi'][j] * self.v[i, j] for j in self.j_range]) for i in
             self.i_range])
...
class OptimizeMapPartitionDistance(object):
    @staticmethod
